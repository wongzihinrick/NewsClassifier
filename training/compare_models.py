...
COMPLEMENT_NB_RESULT_PATH = PROJECT_ROOT / "results" / "complement_naive_bayes_results.csv"
...
def main():
    ...
    if not os.path.exists(COMPLEMENT_NB_RESULT_PATH):
        print("Missing Complement Naive Bayes result file.")
        print("Please run: python training/train_complement_naive_bayes_model.py")
        return

    svm_results = pd.read_csv(SVM_RESULT_PATH)
    logistic_results = pd.read_csv(LOGISTIC_RESULT_PATH)
    complement_nb_results = pd.read_csv(COMPLEMENT_NB_RESULT_PATH)
    
    comparison_df = pd.concat([svm_results, logistic_results, complement_nb_results], ignore_index=True)
